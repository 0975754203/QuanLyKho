BEGIN TRY
    ...
    DECLARE @VatTu TABLE (
        ...
        MaSanPhamUuTien NVARCHAR(50) NULL
    );
    ...
    UPDATE @VatTu
    SET MaSanPhamUuTien = N'VTN030'
    WHERE TenVatTu = N'T - Inox Ø 20 1/2';

    UPDATE @VatTu
    SET MaSanPhamUuTien = N'VTN031'
    WHERE TenVatTu = N'T - Inox Ø 25 1/2';
    ...
    ;WITH KiemTraKhop AS (
        SELECT
            ...
            v.TenVatTu,
            v.SoLuong,
            COUNT(sp.Id) AS SoDongKhop
        FROM @VatTu v
        LEFT JOIN KhoSanPham sp
            ON (
                v.MaSanPhamUuTien IS NOT NULL
                AND sp.MaSanPham = v.MaSanPhamUuTien
            )
            OR (
                v.MaSanPhamUuTien IS NULL
                AND UPPER(LTRIM(RTRIM(REPLACE(REPLACE(REPLACE(sp.TenSanPham, CHAR(9), N' '), N'  ', N' '), N'  ', N' ')))) =
                    UPPER(LTRIM(RTRIM(REPLACE(REPLACE(REPLACE(v.TenVatTu, CHAR(9), N' '), N'  ', N' '), N'  ', N' '))))
            )
        GROUP BY v.STT, v.TenVatTu, v.SoLuong
    )
    SELECT STT, TenVatTu, SoLuong, SoDongKhop
    INTO #VatTuKhongKhop
    FROM KiemTraKhop
    WHERE SoDongKhop <> 1;
    ...
    BEGIN
        SELECT
            ...
            SoDongKhop,
            CASE
                WHEN SoDongKhop = 0 THEN N'Không tìm thấy tên sản phẩm trong KhoSanPham'
                WHEN SoDongKhop > 1 THEN N'Tên sản phẩm bị trùng nhiều dòng trong KhoSanPham'
                ELSE N'Cần kiểm tra lại'
            END AS LyDo
        FROM #VatTuKhongKhop
        ORDER BY STT;
        ...
        SELECT
            loi.STT,
            loi.TenVatTu AS TenVatTuCanNhap,
            loi.SoLuong,
            goiY.MaSanPham AS MaSanPhamGoiY,
            goiY.TenSanPham AS TenSanPhamGoiY
        FROM #VatTuKhongKhop loi
        OUTER APPLY (
            SELECT TOP 10 sp.MaSanPham, sp.TenSanPham
            FROM KhoSanPham sp
            WHERE sp.TenSanPham LIKE N'%' + LEFT(loi.TenVatTu, 8) + N'%'
               OR loi.TenVatTu LIKE N'%' + LEFT(sp.TenSanPham, 8) + N'%'
            ORDER BY sp.TenSanPham
        ) goiY
        ORDER BY loi.STT, goiY.TenSanPham;

        ROLLBACK TRANSACTION;
        RETURN;
    END;

    SELECT N'Tất cả vật tư đều khớp đúng 1 sản phẩm theo tên.' AS KetQua;
    ...
    ;WITH MatchSanPham AS (
        ...
        INNER JOIN KhoSanPham sp
            ON UPPER(LTRIM(RTRIM(REPLACE(REPLACE(REPLACE(sp.TenSanPham, CHAR(9), N' '), N'  ', N' '), N'  ', N' ')))) =
               UPPER(LTRIM(RTRIM(REPLACE(REPLACE(REPLACE(v.TenVatTu, CHAR(9), N' '), N'  ', N' '), N'  ', N' '))))
    ),
    ChiTietGom AS (
